...
@Slf4j
public class IotHandlerExceptionResolver extends DefaultHandlerExceptionResolver {
    ...
    public IotHandlerExceptionResolver(ReloadableMessageSource messageSource) {
    ...
    private ModelAndView writeRestResponse(String code, String message, HttpServletRequest request, HttpServletResponse response) {
        try {
            response.getWriter().print(JSON.toJSONString(R.error(code, message)));
        } catch (IOException e) {
